...
tmp_pokemon_name="${filename%.*}"
pokemon_name=$(curl -sS https://wiki.52poke.com/zh-hant/$tmp_pokemon_name | grep -oE "<title>.*</title>" | sed 's/<title>//' | sed 's/<\/title>//' | sed 's/\ \-\ 神奇寶貝百科，//' | sed 's/\ \-\ 神奇宝贝百科//' | sed 's/關於寶可夢的百科全書//')

# Call cowsay or cowthink.
if [ -n "$THINK" ]; then
	cowthink -f "$pokemon_cow" $word_wrap $pokemon_name
else
	cowsay -f "$pokemon_cow" $word_wrap $pokemon_name
fi
